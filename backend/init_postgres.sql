...
CREATE TABLE user_training_categories (
...
--
-- CREATE OR REPLACE FUNCTION add_default_description()
-- RETURNS TRIGGER AS $$
-- BEGIN
--   IF NOT EXISTS (SELECT 1 FROM descriptions WHERE user_id = NEW.id) THEN
...
--     FOR EACH ROW
--     EXECUTE FUNCTION add_default_description();
